...
public class CGridDrivenContentsGenerator : MonoBehaviour {
    ...
    private int numOfChamberInWidth;
    private int numOfChamberInHeight;
    ...
    public void InitGenerator(float width, float height, int numOfWidth, int numOfHeight)
    {
        chamberWidth = width;
        chamberHeight = height;
        numOfChamberInWidth = numOfWidth;
        numOfChamberInHeight = numOfHeight;
        makeNoneChamber();
    }
    // 맵 구동기 가동
    ...
    {
        for(int i=0; i<numOfChamberInHeight; i++)
        {
            for(int j=0; j<numOfChamberInWidth; j++)
            {
    ...
    private void makeEssentialPath()
    {
        ...
        // 필수 경로 시작 지점
        currentPosition = startChamber = new Vector2Int((int)Random.Range(0.0f, numOfChamberInHeight), 0);
        ...
        // 그리드의 맨 오른쪽 위치 까지 진행
        while (currentPosition.x != numOfChamberInWidth - 1)
        {
            adjacentPosition = getAdjacentPath(currentPosition);
            nextPosition = adjacentPosition[(int)Random.Range(0.0f, adjacentPosition.Length)];
            chamberPosition[nextPosition].SetChamberType(EChamberType.Essential);
